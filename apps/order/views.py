...
# 悲观锁：执行的时候加琐 用户抢锁, 冲突比较多的时候使用悲观锁
...
# 开启乐观锁需要设在mysql事务的隔离性级别, 冲突比较少的时候使用乐观锁
# 乐观锁重复操作的代价比较大时使用悲观锁
        ...
        try:
            ...
            for sku_id in sku_ids:
                for i in range(3):
                    ...
                    # print('user:%d, times:%d, skock:%d' % (user.id, i, orgin_stock))
                    # import time
                    # time.sleep(10)
...
class CommentView(LoginRequiredMixin, View):
    ...
    def get(self, request, order_id):
        ...
        # 使用模板
        return render(request, "dailyfresh/order_comment.html", {"order": order})
